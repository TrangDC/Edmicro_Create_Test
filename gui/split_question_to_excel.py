...
def process_excel_for_correct_answer(excel_file):
    """Process the excel file to extract and add correct answer"""
    try:
        wb = openpyxl.load_workbook(excel_file)
        for sheet_name in wb.sheetnames:
             if sheet_name == "prompt_template" or sheet_name == "tables":
                continue
             ws = wb[sheet_name]
             for row in range (2, ws.max_row +1):
                question_cell = ws.cell(row=row, column=1)
                if question_cell and question_cell.value:
                     question_text = question_cell.value
                     match = re.search(r"^(.*)(?:\s*[A-D]\.\s*[^\n]*\s*)?(Lời giải\s*[:.]?\s*)(.*)$", question_text, re.IGNORECASE | re.DOTALL)
                     if match:
                       correct_answer = match.group(3).strip()
                       question_text = match.group(1).strip()
                       ws.cell(row=row, column=1, value=question_text)
                       ws.cell(row=row, column=4, value=correct_answer)
        wb.save(excel_file)
        logger.info(f"Successfully processed excel file {excel_file} for correct answer")
    except Exception as e:
